...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = - outsideVectors[outsideWordIdx] + np.sum(np.expand_dims(probs, 1) * outsideVectors, axis=0)

    probs[outsideWordIdx] = probs[outsideWordIdx] - 1
    gradOutsideVecs = np.expand_dims(probs, 1) * centerWordVec
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    loss = - np.log(sigmoid(np.dot(outsideVectors[indices[0]], centerWordVec))) - \
           np.sum(np.log(sigmoid(-np.dot(outsideVectors[indices[1:]], centerWordVec))))
    dsigmoid = lambda x: (1 - sigmoid(x))
    gradCenterVecPos = dsigmoid(np.dot(outsideVectors[indices[0]], centerWordVec)) * outsideVectors[indices[0]]
    gradCenterVecNeg = np.sum(np.expand_dims(dsigmoid(-np.dot(outsideVectors[indices[1:]], centerWordVec)), 1) * outsideVectors[indices[1:]], axis=0)
    gradCenterVec = - gradCenterVecPos + gradCenterVecNeg

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = - dsigmoid(np.dot(outsideVectors[indices[0]], centerWordVec)) * centerWordVec
    for i in range(K):
        gradOutsideVecs[indices[i+1]] += dsigmoid(-np.dot(outsideVectors[indices[i+1]], centerWordVec)) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        currentLoss, currentGradCenterVec, currentGradOutsideVecs = \
            word2vecLossAndGradient(centerWordVec,
                                    outsideWordIdx,
