...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i, transition in enumerate(transitions):
            mini_batch[i].parse([transition])
            if len(mini_batch[i].stack) == 1 and len(mini_batch[i].buffer) == 0:
                unfinished_parses = [p for p in unfinished_parses if p != mini_batch[i]]
