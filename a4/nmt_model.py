...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_packed = nn.utils.rnn.pack_padded_sequence(X, torch.Tensor(source_lengths), enforce_sorted=False)
        ...
        last_hidden_flatten = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(last_hidden_flatten)
        last_cell_flatten = torch.cat((last_cell[0], last_cell[1]), dim=1)
        init_decoder_cell = self.c_projection(last_cell_flatten)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_split = torch.split(Y, [1]*Y.shape[0])
        for Y_t in Y_split:
