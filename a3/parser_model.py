...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        emb = torch.index_select(self.embeddings, 0, w.flatten())
        emb_reshaped = emb.view(w.shape[0], -1, self.embeddings.shape[1])
        x = emb_reshaped.view(w.shape[0], -1)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        emb = self.embedding_lookup(w)  # (batch_size, n_features * embed_size)
        emb_to_hidden_out = torch.matmul(emb, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        h = torch.nn.functional.relu(emb_to_hidden_out)
