...
if args.function == 'pretrain':
    ...
    tconf = TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3, lr_decay=True,
    ...
    #         num_workers=4
    from trainer import Trainer, TrainerConfig
    finetuning_text = open(args.finetune_corpus_path, 'r').read()
    finetuning_dataset = dataset.NameDataset(pretrain_dataset, finetuning_text)
    tconf = TrainerConfig(max_epochs=75, batch_size=256, learning_rate=6e-4, lr_decay=True,
                          warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                          num_workers=0)
    trainer = Trainer(model, finetuning_dataset, None, tconf)
    trainer.train()
    torch.save(model.state_dict(), args.writing_params_path)
